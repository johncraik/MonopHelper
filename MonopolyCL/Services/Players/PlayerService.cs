...
public class PlayerService
{
    ...
    public PlayerService(GameDbSet<PlayerDM> playerSet, UserInfo userInfo)
    {
    ...
    public async Task<List<PlayerDM>> GetPlayers() =>
        await _playerSet.Query().Where(p => p.UserId == _userInfo.UserId).ToListAsync();
    
    public async Task<ValidationResponse> TryAddPlayer(PlayerDM player)
    {
        var inUse = await _playerSet.Query().AnyAsync(p => p.Name == player.Name && p.UserId == _userInfo.UserId);
